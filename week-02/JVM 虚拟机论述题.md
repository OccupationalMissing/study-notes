...
* 运行时常量池（字符串常量池）  
运行时常量池在方法区中，是每一个类或者接口的常量池的运行时的表现形式。方法区中会为每个class保存以下数据：运行时常量池、类的属性、类的方法、方法和构造器的代码。其中，字符串常量池简单来说就是专门针对String类型设计的常量池，其使用 HashTable 存放数据， HashTable 的本质是一个数组。
...
#### 解释一个对象的创建过程
1. new指令
2. 类加载检查 (检查类是否已被加载，若未加载则进入类加载流程)
3. 分配内存空间
4. 内存空间初始化
5. 必要信息设置
6. 执行init方法
...
#### 解释一个对象的内存分配  
1. 申请内存空间
2. 检查Eden区空间是否充足可分配，若分配不成，新生代 MinorGC 。之后再次检查新生代是否可分配空间，若满足条件则进入第3步，不满足进入第4步。
3. 新生代分配内存空间
4. 判断是否进入老年代，若不满足，触发 Full GC 。 之后检查老年代是否可分配空间，若满足条件进入第5步，不满足进入第6步。
5. 老年代分配内存空间
6. OOM

#### 解释一个对象的销毁过程
> 判断一个对象是否是垃圾的算法：
> 1. 引用计数法： 被引用的对象则加一，当对象计数归零时，则当前对象不再被应用，可以被归入可回收范围。此方法无法避免类的循环引用，当出现循环引用时，多个对象的引用计数一直无法归零，也无法被回收。
> 2. 可达性分析法 (根可达算法)： 通过一系列的名为 GCRoot 的对象作为起始点，从这些节点开始向下搜索，搜索所走过的路径称为引用链。当一个对象到 GCRoot 没有任何引用链相连时，则证明此对象是不可用的，也就是不可达的，可被纳入待回收范畴。  

在 Java 中，垃圾回收的时机是由 JVM 决定的，而非由程序员手动控制。JVM 通过可达性分析法判断当前对象是否需要被回收。即使在可达性分析算法中不可达的对象，也并非是一定会消亡，而是至少会经历两次标记过程。通过可达性分析法，第一次标记时会标记不存在与 GC Roots 相连接的引用链的对象。经历第一次标记后，会再次做一次筛选，判断对象是否有必要执行 finalize() 方法。在 finalize() 方法中没有重新与引用链建立关联关系的，将被标记未可回收。两次标记流程后，被标记的对象将被 JVM 回收，否则会继续存活。
...
#### 为什么需要内存担保？
当新生代无法分配内存的时候，需要将新生代迁移到老年代，然后才能将新对象放置到新生代。其根本目的在于保证大对象不会因为新生代的内存不足而导致无处安放。
...
2. 标记-复制算法：简称复制算法。  
* 将内存一分为二，每次只使用其中一半，另一半作为保留区域，根据对象使用情况分别标记
...
6. CMS 收集器  
并行收集器，全称**Concurrent Mark Sweep** ，只回收老年代，采用标记-清除算法。当堆内存使用率达到某一阈值即开始回收，并不等待老年代几乎被填满才收集。GC 过程主要分为4个阶段(初始标记、并发标记、重新标记、并发清除)。
